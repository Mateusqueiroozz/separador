...
//impressora
	$consultaimp= mysql_query("SELECT * FROM impressora WHERE id='".$impressora."'");
	$linhaimp = mysql_fetch_array($consultaimp);
...
<div class="btn-group pull-right" style="position: absolute;top: 0; right: 0">
	...
	<a href="sair.php" class="ui-btn ui-corner-all ui-btn-inline ui-mini footer-button-left ui-btn-icon-left ui-icon-power" data-ajax="false">Sair</a>
...
<label for="text-basic" style="text-align:center; font-size:12px; ">CADASTRO DE SESSÃO (GONDOLA)</label>
...
echo "<div class='sim'>";	
	...
	$consultaprd = mysql_query("SELECT * FROM produto WHERE(codigo1='".$codigo."' OR codigo2='".$codigo."') ORDER BY valor DESC LIMIT 1");
	
	
	while ($linhaprd = mysql_fetch_array($consultaprd)){
		//echo '<br>';
		echo '
			...
			<div class="ui-corner-all custom-corners">
				<div class="ui-bar ui-bar-a" style="text-align:center;">
					...
					$consultaprdant = mysql_query("SELECT
												impressao.id,
												impressao.sessao,
					...
					$linhaprdant = mysql_fetch_array($consultaprdant);
...
ORDER BY id desc");
	while ($linhaimp = mysql_fetch_array($consultaimp)){
		$ordem ++;
	?>
